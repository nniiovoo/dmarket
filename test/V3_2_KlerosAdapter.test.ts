...
describe("V3.2 KlerosV2DisputeAdapterV3_2", function () {
  ...
  async function deploy() {
    ...
    // Re-enable the test token after ownership transfer. setAcceptedToken is
    // owner-only on the marketplace, so we have to go through the adapter's
    // executeOnMarketplace pass-through.
    const tokenAddr = await mockUsd.getAddress();
    const setTokenData = marketplace.interface.encodeFunctionData("setAcceptedToken", [tokenAddr, true]);
    await adapter.connect(owner).executeOnMarketplace(setTokenData);

    // Mint mUSD to the test buyer.
    await mockUsd.mint(buyer.address, AMOUNT_RAW * 10n);

    return { ethers, owner, buyer, seller, other, marketplace, mockUsd, arbitrator, adapter };
  }
  ...
  describe("constructor + ownership handover", function () {
    ...
    it("rejects zero marketplace address", async function () {
      ...
      const factory = await ethers.getContractFactory("KlerosV2DisputeAdapterV3_2", deployer);
      await expect(
    ...
    it("rejects zero arbitrator address", async function () {
      const { ethers } = await network.create();
      const [deployer] = await ethers.getSigners();
      const marketplace = await ethers.deployContract("EscrowMarketplaceERC20", [], deployer);
      const factory = await ethers.getContractFactory("KlerosV2DisputeAdapterV3_2", deployer);
      await expect(
  ...
  describe("rule (Kleros callback)", function () {
    ...
    it("ruling=2 seller wins → release (after cooldown)", async function () {
      ...
      expect(sellerBalAfter - sellerBalBefore).to.equal(AMOUNT_RAW);
    });
  ...
  describe("emergency refund (propose / execute / cancel)", function () {
    ...
    it("execute with refundBuyer=false releases to seller", async function () {
      const ctx = await deploy();
      const orderId = await escalated(ctx);
  ...
  describe("conservation of marketplace token balance", function () {
    ...
    it("seller-wins ruling: marketplace token delta == -amount", async function () {
      const ctx = await deploy();
      const orderId = await setupDisputedOrder(ctx);
      const { buyer, adapter, arbitrator, marketplace, mockUsd } = ctx;
      const fee = await arbitrator.arbitrationCost("0x");
