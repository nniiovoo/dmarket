...
// Native-only amount label. Other marketplace variants render under
// their own routes and never reach this client component.
function formatOrderAmount(order: OrderView): string {
  return `${formatAmount(order.amount)} ETH / MATIC`;
}
...
export default function OrderDetailClient() {
  ...
  // The server wrapper in page.tsx redirects any non-native marketplace
  // hint before this component renders. This component is the v3 / v3.1
  // render path only.
  const orderApi = useQuery({
    queryKey: ["order", targetChainId, orderId?.toString()],
    queryFn: () => fetchOrder(targetChainId, orderId?.toString() ?? ""),
    enabled: orderId !== undefined && supported,
    refetchInterval,
    retry: false
  });
  ...
  const orderQuery = useReadContract({
    ...
    query: {
      ...
      refetchInterval: chainPollInterval,
      retry: false
    }
  });
  ...
  const amountLabel = order === undefined ? "-" : formatOrderAmount(order);
  const actions =
    order === undefined
      ? { available: [], locked: [] }
      : computeActions({
        ...
        <>
          ...
          <Card title="Order details" action={<StatusBadge status={order.status} />}>
            <div className="grid gap-3 text-sm md:grid-cols-2">
              <PartyInfo label="Buyer" address={order.buyer} chainId={targetChainId} />
              <PartyInfo label="Seller" address={order.seller} chainId={targetChainId} />
...
// Same shape as <Info /> for buyer/seller, with a compact reputation badge
// appended. computeSellerScore is address-agnostic so the same component
// works for both roles (the score formula penalises participants who
// generate disputes / refunds regardless of which side they were on).
