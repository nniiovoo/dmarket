...
import { klerosAdapterV3_3Abi } from "@/lib/contractsV3_3";
...
// reading from chain because we use it only for UX formatting. Same
// value as v3.2 (3 days).
const DISPUTE_RESOLUTION_DELAY_SECONDS = 3 * 24 * 60 * 60;

export function V3_3KlerosSection({ order, chainId, adapterAddress, role }: Props) {
  const router = useRouter();
  const orderIdBigInt = BigInt(order.onChainOrderId);

  const klerosIdQuery = useReadContract({
    address: adapterAddress,
    abi: klerosAdapterV3_3Abi,
    chainId,
