...
async function main() {
  ...
  const marketplaceOverrides = isAmoy ? { gasLimit: 5_000_000n } : {};
  ...
  console.log("Deploying ChainUs v3.1...");
  ...
  console.log("V3.1 vault address:", vaultAddress);
  console.log("V3.1 vault deployment tx hash:", vaultDeploymentTx.hash);

  const MarketplaceFactory = await ethers.getContractFactory("EscrowMarketplaceV3_1", deployer);
  ...
  console.log("V3.1 marketplace address:", marketplaceAddress);
  console.log("V3.1 marketplace deployment tx hash:", marketplaceDeploymentTx.hash);
  ...
  console.log("V3.1 vault wiring tx hash:", wiringTx.hash);
  console.log("V3.1 vault marketplace:", await vault.marketplace());
  console.log(`V3_1_${envNetwork}_VAULT_ADDRESS=${vaultAddress}`);
  console.log(`V3_1_${envNetwork}_MARKETPLACE_ADDRESS=${marketplaceAddress}`);
