// Migrate v3.2 marketplace ownership to KlerosV2DisputeAdapterV3_2.
//
...
//     setAcceptedToken) are reachable via adapter.executeOnMarketplace.
//
// Required env:
//   PRIVATE_KEY                                — current marketplace owner
//   ARBITRUM_SEPOLIA_RPC_URL                   — or matching RPC
//   V3_2_<NETWORK>_MARKETPLACE_ADDRESS
//   V3_2_<NETWORK>_KLEROS_ADAPTER_ADDRESS      — from deployKlerosAdapterV3_2 output
//
...
async function main() {
  ...
  const adapterAddress = requireEnv(`V3_2_${upper}_KLEROS_ADAPTER_ADDRESS`);

  console.log("Migrating v3.2 marketplace ownership → Kleros adapter");
  console.log(`  network:     ${networkName}`);
  console.log(`  signer:      ${signerAddr}`);
  console.log(`  marketplace: ${marketplaceAddress}`);
  console.log(`  adapter:     ${adapterAddress}`);

  const marketplace = await ethers.getContractAt("EscrowMarketplaceERC20", marketplaceAddress, signer);
  const adapter = await ethers.getContractAt("KlerosV2DisputeAdapterV3_2", adapterAddress, signer);
