...
async function main() {
  ...
  await logStep(ethers, "markShipped", shippedTx.hash, orderId, "Shipped", contractAddress);

  const receivedTx = await marketplace.confirmReceived(orderId);
  await receivedTx.wait();
  await assertStatus(marketplace, orderId, 3);
  await logStep(ethers, "confirmReceived", receivedTx.hash, orderId, "Completed", contractAddress);
